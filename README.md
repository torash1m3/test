...
- `users/{userId}` — приватный профиль пользователя, email и внутренние поля
- `publicProfiles/{userId}` — публичная карточка профиля без email
...
- Публичные страницы профилей должны читать `publicProfiles`, а не приватный `users`.
